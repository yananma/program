...
1. 删除全部：`ggdG`  
2. 从第 3 行开始删：`3ggdG`
3. 从当前位置删除到第 20 行：`d20gg`
4. 从当前位置删到底：`dG`
5. 从当前位置删到开头：`dgg`  
6. 删除 5 到 11 行：`:5,11d`  
7. `:m,ny<Enter>` 复制 m 行到 n 行的内容，复制很长的内容使用
8. V  5J 选 5 行，或者从当前行到 23 行，V 23gg  
9. 缩进 `>>` 和 . 结合使用   
10. Ctrl + v 进入 block 模式    
...
vim
:set paste
进入插入模式（按 i）并粘贴内容（Ctrl+Shift+V 或鼠标右键）。

粘贴后 退出插入模式（按 Esc），然后关闭粘贴模式：

vim
:set nopaste
原理：paste 模式会临时禁用自动缩进、格式化和插件干扰。      
