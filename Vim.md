...
3. 搜索文件 `:/单词` 查找下一个，n next，上一个 N；读源码，查函数位置；HTML 中找某一个标签。在比较大的文档中，查找命令是非常经济实惠的移动方法
4. `j` 向下，`k` 向上；扑克牌 j 在下面，k 在上面  
5. 0 行首（左对齐）、 ^ 第一个非空白字符(像一顶蓑笠一样，有人在下面，才会戴蓑笠)、$ 最后一个、w word 下一个、b backward 上一个、e end 词尾、ge 反向词尾、
6. gg 第一行、3 gg 第三行、当前屏最后一行：`Shift+l` Last、G 最后一行、   
...
2. 从第 3 行开始删：`3ggdG`  
3. 删除 5 到 11 行：`:5,11d`  
4. `:m,ny<Enter>` 复制 m 行到 n 行的内容，复制很长的内容使用
5. V  5J 选 5 行，或者从当前行到 23 行，V 23gg  
6. 缩进 > 空格  
7. 
